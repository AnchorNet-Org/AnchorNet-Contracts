...
category below using the Keep a Changelog
style and concise bullet points.

Added
Anchors: regression coverage pinning that deregister_anchor leaves
balance / anchor_balances / pool.providers untouched, that
re-registering the same anchor restores those views exactly (no reset, no
double-count), that the re-registered anchor can withdraw its preserved
balance without providing liquidity again first, and that
provide_liquidity / open_settlement stay blocked with
AnchorNotRegistered while the anchor is deregistered
(test_deregister_re_register_preserves_balances_and_provider_count).
Liquidity: restored the is_min_liquidity_configured(asset) read view
(documented in 
ADMIN.md
), distinguishing an explicit 0 floor from
a never-configured asset.
Settlements: restored the oldest_pending_settlement_id(asset) read
view, returning the lowest id among currently Pending settlements for an
asset so off-chain keepers can locate the oldest settlement to check for
expiry.
Changed
Deprecated
Removed
Fixed
Tests: repaired the broken test build so the full suite compiles and
runs green again: linked alloc for the #![no_std] test module, fixed
quote_fee client call sites to the generated client's unwrapped i128
return, replaced an unavailable std::collections::HashSet import, fixed
the ContractEvents assertion in test_clear_max_settlement_amount, and
corrected test_clear_operator to expect a contract-level
Error::NotAuthorized (restoring the blanket auth mock before further
admin-authorized calls).
Security
[0.9.0]
Added
