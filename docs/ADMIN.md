...
Within src/lib.rs, two internal helper functions enforce these roles:
...
Below is a complete matrix of every entrypoint in the contract that is restricted to either the Admin or Operator, including their auth requirements and the events they emit from src/events.rs.

Function Name	Required Authorization	Emitted Event (Topic / Data)	Cross-References	Description
initialize	None (Callable once)	("init",) / admin	events::initialized	Sets the initial administrator of the contract.
set_admin	Admin	("admin",) / new_admin	events::admin_changed	Transfers administrative control directly to new_admin in a single step.
...
register_anchor	Admin	("anchor", anchor) / ()	events::anchor_registered	Registers an anchor as authorized.
register_anchors	Admin	Multiple ("anchor", anchor) / ()	events::anchor_registered	Registers a batch of anchors.
deregister_anchor	Admin	("deanchor", anchor) / ()	events::anchor_removed	Removes an anchor from the registry.
set_min_liquidity	Admin	("minliq", asset) / floor	events::min_liquidity_changed	Sets the minimum liquidity floor for a pool; 0 intentionally disables the check while still leaving a configured entry visible via is_min_liquidity_configured.
set_max_settlement_amount	Admin	("maxamt", asset) / amount	events::max_settlement_amount_changed	Caps the settlement reserve amount for an asset.
set_fee	Admin	("fee",) / bps	events::fee_changed	Configures the global protocol fee (bps).
set_asset_fee	Admin	("assetfee", asset) / bps	events::asset_fee_changed	Sets a fee override for a specific asset.
clear_asset_fee	Admin	("feeclear", asset) / ()	events::asset_fee_cleared	Removes an asset fee override.
collect_fees	Admin	("collect", asset) / amount	events::fees_collected	Withdraws collected protocol fees.
set_fee_waiver	Admin	("waiver", anchor) / waived	events::fee_waiver_changed	Grants/revokes anchor fee waivers.
set_settlement_expiry_ledgers	Admin	("expiry",) / ledgers	events::settlement_expiry_changed	Configures settlement expiry window.
execute_settlement	Admin	("executed", id) / ()	events::settlement_executed	Finalizes a settlement.
Admin Transfer Lifecycle
Administrator transfers can happen in two ways:

Single-Step Transfer (set_admin): Instantly updates the admin role.
...
Two-Step Transfer (propose_admin -> accept_admin): Guarantees that the new administrator is active and holds the private key matching the proposed address.
