...
EVENTS.md
 – event topics, argument types, and indexer integration guide
...
is_min_liquidity_configured(asset)	–	Read whether a minimum liquidity floor has ever been configured for an asset, distinguishing an explicit 0 floor from no configuration at all
...
oldest_pending_settlement_id(asset) – Read the lowest (oldest) id among currently Pending settlements for an asset, or None; intended for keepers deciding what to check for expiry
...
Events
For detailed event documentation, including argument shapes, emission sites, and indexer integration guidance, see 
...
If modifying public contract functions, parameters, return types, data structures, error codes, or events, review and complete the 
